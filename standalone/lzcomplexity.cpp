...
lz::lz_int process(lz_options& opt) {
  ...
  if (opt.verbose) {
    ...
    if (opt.warn_out) std::cout << std::endl;
  }
  ...
  if (opt.verbose) {
    ...
    for (auto x: lz.data) std::cout << x.getComplexity() << " ";
    std::cout << std::endl;
  ...
  if (opt.verbose) {
    ...
    for (auto x: lz.data) std::cout << x.getEntropyDensity() << " ";
    std::cout << std::endl;
    std::cout << print_msg(MSG::INFO, "Finished in: " + std::to_string(duration(end_time - init_time)) + " s")
              << std::endl
              << std::endl;
  ...
  if (opt.entropy_density) {
    ...
    if (opt.verbose) {
      std::cout << print_msg(MSG::INFO,
                             "Total time elapsed: " + std::to_string(duration(g_end - g_now)) + " s")
                << std::endl;
    }
    lz::utils::DisabledMT();
    return EXIT_SUCCESS;
    ...
    if (opt.verbose) {
      ...
      for (auto i = 0ul; i < lz.data.size(); i++) {
        ...
        if (x.getRandomShuffleComplexity().summands.size() > 0) {
          std::cout << print_msg(MSG::INFO,
                                 "Shuffle entropy terms of line: " + std::to_string(i + 1) + " [ ");
          for (auto t: x.getRandomShuffleComplexity().summands) std::cout << t << " ";
          std::cout << "]\n";
        }
      }
      std::cout << print_msg(MSG::INFO, "Multi information: ");
      for (auto x: lz.data) std::cout << x.getRandomShuffleComplexity().multi_information << " ";
      std::cout << std::endl;
    ...
    if (opt.verbose) {
      ...
      for (auto i = 0ul; i < lz.data.size(); i++) {
        ...
        if (x.getPairedShuffleComplexity().summands.size() > 0) {
          ...
          for (auto t: x.getPairedShuffleComplexity().summands) std::cout << t << " ";
          std::cout << "]\n";
        }
      }
      std::cout << print_msg(MSG::INFO, "Multi information: ");
      for (auto x: lz.data) std::cout << x.getPairedShuffleComplexity().multi_information << " ";
      std::cout << std::endl;
  ...
  if (opt.mixed_entropy) {
    ...
    if (opt.verbose) {
      ...
      for (auto x: lz.mixed_entropy_density) std::cout << x << " ";
      std::cout << std::endl;
      std::cout << print_msg(MSG::INFO,
                             "Finished in: " + std::to_string(duration(end_time - init_time)) + " s")
                << std::endl
