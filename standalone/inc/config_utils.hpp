 ...
 */
[[nodiscard]] inline std::vector<std::string> split(std::string_view s, char delim, bool skip_empty = true) {
...
inline void read_csv(const fs::path& path, 
   ...
   while ((line = input.next_line()) != nullptr) {
      auto cols = split(line, delimiter, false);
 ...
 */
[[nodiscard]] inline std::vector<std::pair<std::string, std::vector<int>>> 
read_csv_with_header(const fs::path& path) {
...
inline void read_dna(const fs::path& path, std::vector<lz::sequence>& data, bool multiline = false) {
   ...
   auto reader = kseq::kseq_open(fp, [](auto& f, char* buf, std::size_t size) {
 ...
 */
inline void parse_by_format(const fs::path& path, std::vector<lz::sequence>& seqs, MagickNumber format, bool multiline = false) {
   lz::utils::pnm parser;
 ...
 */
inline void process_file(const fs::path& path, std::vector<lz::sequence>& seq, MagickNumber format, bool multiline = false) {   
   if (seq.empty()) {
      seq.emplace_back();
   }
 ...
 */
[[nodiscard]] inline std::vector<lz::sequence> 
...
public:
   explicit DirectoryReader(const fs::path& path, 
                           std::vector<std::string> exclude_ext = {".json", ".log"})
      : dir_path_(path.string())
      , exclude_extensions_(std::move(exclude_ext)) {
