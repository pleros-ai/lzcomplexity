...
namespace detail {
  ...
  inline constexpr std::string_view Header = "LempelZiv-76 complexity engine. Suited for "
                                             "complexity analysis of time series. Send bug reports to "
                                             "estevez@fisica.uh.cu or "
  ...
  inline constexpr std::array<CmdOpt, 17> OptList{
    {{"alphabet",
     ...
     {"output",
      "o,output",
      "Output filename. Default appends to the end of input file a .json "
      "extension"},
  ...
  constexpr const CmdOpt* findOpt(std::string_view key) noexcept {
  ...
  inline constexpr std::string_view WarnDataSize
    = "Ignoring parallel processing because data source is too short";
...
struct lz_options {
  ...
  lz::lz_int excess_init_line
    = lz::details::UNDEFINED_LINES;  //?> Initial line where get shuffle entropy deficit
                                     // terms ?  (valid for excess of entropy by shuffling).
  lz::lz_int excess_end_line = lz::details::UNDEFINED_LINES;  //?> Final line where get shuffle entropy
                                                              // deficit ? terms (valid for excess of
                                                              // entropy by shuffling).
  lz::lz_uint  n_jobs = 1;
  ...
  explicit lz_options(const cxxopts::parse_result& result) {
    ...
    args.alphabet
      = result.count("alphabet") ? std::stoi(result["alphabet"].as<std::string>()) : lz::details::NO_ALPHABET;
  ...
  void parseExcessOptions(const std::vector<std::string>& excess_args) {
    ...
    if (excess_args.size() > factor_idx) {
      ...
      if (excess_args.size() <= line_start_idx) {
        ...
        excess_end_line = lz::details::ALL_LINES;
      } else {
...
#ifdef PLEROS_INTERNAL
  if (json_opt.contains("get_paired_shuffle")) opt.get_paired_shuffle = json_opt["get_paired_shuffle"];
#endif
  // Arguments for LZ76 algoeithms
  if (json_opt.contains("partitions")) opt.args.chunks = json_opt["partitions"];
  if (json_opt.contains("block_size")) opt.args.block_size = json_opt["block_size"];
  if (json_opt.contains("initial_line")) opt.excess_init_line = json_opt["initial_line"];
  if (json_opt.contains("final_line")) opt.excess_end_line = json_opt["final_line"];
  if (json_opt.contains("get_shuffle_terms")) opt.args.get_shuffle_terms = json_opt["get_shuffle_terms"];

  if (json_opt.contains("logs")) opt.verbose = json_opt["logs"];
  else opt.verbose = false;
}
...
inline cxxopts::options generateOptions() {
  ...
  options.custom_help("[OPTIONS] <file>").set_width(120).set_tab_expansion(true);
  ...
  // Complex options
  g(getOptValue("excess_opt"),
    getOptDesc("excess_opt"),
