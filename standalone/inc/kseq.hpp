...
public:
   ...
   std::string str() const {
      return s ? std::string(s, l) : std::string{};
   }
...
public:
   ...
   public:
      ...
      explicit iterator(kseq_t* seq) noexcept : seq_(seq) {
 ...
 */
struct PosixReader {
   auto operator()(int fd, char* buf, std::size_t len) const noexcept {
...
struct FuncReader {
   ...
   explicit FuncReader(Func f) : func(std::move(f)) {}
 ...
 */
inline auto kseq_open(int fd) {
   return std::make_unique<kseq_fd>(fd, PosixReader{});
}
 ...
 * @return Unique pointer to kseq reader
 */
inline auto kseq_open(std::FILE* fp) {
