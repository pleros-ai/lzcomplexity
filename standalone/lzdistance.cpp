...
void save_data(lz::dist::LZ_Flags& flags, lz::dist::LZ_Output& results, lz_options& opt) {
  ...
  if (opt.matrix_threshold) {
    out_data["directed_matrix"] = results.directed_matrix;
  }
...
lz::lz_int process(lz_options& opt) {
  ...
  if (opt.verbose) {
    ...
    if (!opt.second_input.empty() || !opt.second_input_dir.empty())
      std::cout << lz::GREEN_COLOR << " • " << lz::BLUE_COLOR
                << (opt.is_second_directory ? "[ directory ] " : "[ file ] ") << lz::END_COLOR
                << lz::END_COLOR << (opt.is_second_directory ? opt.second_input_dir : opt.second_input)
                << std::endl
                << std::endl;
  }
  ...
  auto load_second_data = [&second_data, &opt]() {
    if (opt.second_input.empty() && opt.second_input_dir.empty()) return;
  ...
  flags.first_range = opt.is_second_directory ? opt.first_dir_range : opt.first_range;
  flags.second_range = opt.is_second_directory ? opt.second_dir_range : opt.second_range;
  ...
  if (opt.matrix_threshold != std::numeric_limits<lz::lz_int>::max()) {
    if (opt.verbose) {
      std::cout << lz::GREEN_COLOR << "2." << verbose_index++ << lz::END_COLOR
                << ". Calculating direct graph\n";
      init_time = now();
    }
    lz::lz76DirectedMatrix(flags, lz);
    if (opt.verbose) {
      const auto end_time = now();
      std::cout << print_msg(MSG::INFO, "Direct graph: ");
      for (auto i = 0ul; i < lz.directed_matrix.size(); i++) {
        if (i > 0) std::cout << std::string(24, ' ');
        for (auto j = 0ul; j < lz.directed_matrix[i].size(); j++) {
          std::cout << lz.directed_matrix[i][j] << " ";
        }
        std::cout << std::endl;
      }
  ...
  auto g_end = now();
  if (opt.verbose) {
    std::cout << lz::GREEN_COLOR << "2." << verbose_index++ << ". Saving results in: " << opt.output
              << std::endl
              << lz::END_COLOR;
    init_time = now();
  }
