...
namespace lz {
    namespace suffixarray {
        ...
        CaPS_SA::CaPS_SA(const char* T, lz_int n, lz_int subproblem_count, lz_int max_context)
            ...
            part_ruler_(nullptr) {
            if (n_ < 0 || (n_ > 0 && p_ > n_)) {
                // std::cerr << subproblem_count << " " << n << "The environment variable `PARLAY_NUM_THREADS` needs to be set. Aborting.\n";
        ...
        CaPS_SA::CaPS_SA(CaPS_SA&& other) noexcept :CaPS_SA(std::move(other.T_), other.n_, other.p_, other.max_context) {
        ...
        void CaPS_SA::merge(const lz_int* X, lz_int len_x, const lz_int* Y, lz_int len_y, const lz_int* LCP_x, const lz_int* LCP_y, lz_int* Z, lz_int* LCP_z) const {
        ...
        void CaPS_SA::merge_sort(lz_int* const X, lz_int* const Y, const lz_int n, lz_int* const LCP, lz_int* const W) const {
        ...
        void CaPS_SA::initialize() {
            ...
            const auto t_e = now();
            if (debug) std::cerr << "Initialized required data structures. Time taken: " << duration(t_e - t_s) << " seconds.\n";
        }
        ...
        void CaPS_SA::sort_subarrays() {
            ...
            const auto mem_init = [SA_ = SA_, SA_w = SA_w](const lz_int i) {
                SA_[i] = SA_w[i] = i;
                };
            ...
            const auto t_e = now();
            if (debug) std::cerr << "Sorted the subarrays independently. Time taken: " << duration(t_e - t_s) << " seconds.\n";
        }
        ...
        void CaPS_SA::select_pivots() {
            ...
            auto const temp_1 = utils::allocate<lz_int>(sample_count), temp_2 = utils::allocate<lz_int>(sample_count);
        ...
        void CaPS_SA::locate_pivots(lz_int* const P) const {
            ...
            const auto t_e = now();
            if (debug) std::cerr << "Located the pivots in each sorted subarray. Time taken: " << duration(t_e - t_s) << " seconds.\n";
        }


        lz_int CaPS_SA::upper_bound(const lz_int* const X, const lz_int n, const char* const P, const lz_int P_len) const {
            ...
            lz_int approx = 65536;       // TODO: better tune and document.
        ...
        void CaPS_SA::partition_sub_subarrays(const lz_int* const P) {
            ...
            const auto collate = // Collates the `j`'th sub-subarray from each sorted subarray to partition `j`.
                [&](const lz_int j)
                {
                    ...
                    for (lz_int i = 0; i < p_; ++i) // Subarray `i`.
                    {
                        ...
                        // if (debug) std::cout << "Collect: " << std::to_string(sub_subarr_size) + "< j: " + std::to_string(j) + "<pre " << std::endl;
                        ...
                        // if (debug) std::cout << "Collect: " << std::to_string(sub_subarr_size) + "< j: " + std::to_string(j) + "< " << std::endl;
                    }
            ...
            const auto t_e = now();
            if (debug) std::cerr << "Collated the sorted sub-subarrays into partitions. Time taken: " << duration(t_e - t_s) << " seconds.\n";
        }
        ...
        void CaPS_SA::merge_sub_subarrays() {
            ...
            const auto mem_init =
                [&](const lz_int j)
                {
                    ...
                    std::memcpy(SA_ + part_size_scan_[j], SA_w + part_size_scan_[j], part_size * sizeof(lz_int));
                    std::memcpy(LCP_ + part_size_scan_[j], LCP_w + part_size_scan_[j], part_size * sizeof(lz_int));
                };
            ...
            const auto t_e = now();
            if (debug) std::cerr << "Merged the sorted subarrays in each partition. Time taken: " << duration(t_e - t_s) << " seconds.\n";
        }


        void CaPS_SA::sort_partition(lz_int* const X, lz_int* const Y, const lz_int n, const lz_int* const S, lz_int* const LCP_x, lz_int* const LCP_y) {
            if (n == 1)
                return;
            ...
            (flat_count_l < nested_par_grain_size || flat_count_r < nested_par_grain_size) ? (f(), g()) : lz::utils::par_do(f, g);
        ...
        void CaPS_SA::compute_partition_boundary_lcp() {
            ...
            const auto t_e = now();
            if (debug) std::cerr << "Computed the LCPs at the partition boundaries. Time taken: " << duration(t_e - t_s) << " seconds.\n";
        }
        ...
        void CaPS_SA::clean_up() {
            ...
            const auto t_e = now();
            if (debug) std::cerr << "Released the temporary data structures. Time taken: " << duration(t_e - t_s) << " seconds.\n";
        }
        ...
        utils::LZ_SuffixArray CaPS_SA::construct() {
            ...
            lz_int* const P = utils::allocate<lz_int>(p_ * (p_ + 1)); // Collection of pivot locations in the subarrays.
            locate_pivots(P);
            ...
            if (debug) std::cerr << "Constructed the suffix array. Time taken: " << duration(t_end - t_start) << " seconds.\n";
        ...
        void CaPS_SA::dump(std::ofstream& output) {
            ...
            output.write(reinterpret_cast<const char*>(SA_), n_ * sizeof(lz_int));
        ...
        void CaPS_SA::dump_plain(std::ofstream& output) {
            ...
            const auto t_end = now();
            if (debug) std::cerr << "Plain dumped the suffix array. Time taken: " << duration(t_end - t_start) << " seconds.\n";
        }
