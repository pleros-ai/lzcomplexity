...
namespace lz {
   namespace suffixarray {
      ...
      void CaPS_SA::sort_subarrays() {
         ...
         const auto mem_init = [SA_ = SA_, SA_w = SA_w](const lz_int i) { SA_[i] = SA_w[i] = i; };
      ...
      void CaPS_SA::dump_plain(std::ofstream& output) {
         ...
         const auto t_end = now();
         if (debug)
            std::cerr << "Plain dumped the suffix array. Time taken: " << duration(t_end - t_start) << " seconds.\n";
      }
