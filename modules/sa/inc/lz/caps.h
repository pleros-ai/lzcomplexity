 ...
 ***************************************************************************/
 ...
 /***************************************************************************
  *   Copyright (C) 2023 by Efren Aragon Perez                 						  *
...
namespace lz {
  namespace suffixarray {
    ...
    private:
      ...
      const lz_int pivot_per_part_;  //!> Number of pivots to sample per subarray.
      lz_int* part_size_scan_;       //!> Inclusive scan (prefix sum) of the sizes of the pivoted final partitions containing appropriate sorted sub-subarrays.
      lz_int* part_ruler_;           //!> "Ruler" for the partitions—contains the indices of each sub-subarray in each partition.
      ...
      // static lz_int* allocate(lz_int size) { return static_cast<lz_int*>(std::malloc(size * sizeof(T_))); }
    ...
    public:
      ...
      auto operator()(std::string str) -> utils::LZ_SuffixArray {
        return construct(str);
      }
      auto operator()(const char* str, lz_int n) -> utils::LZ_SuffixArray {
        return construct(str, n);
      }
      ...
      friend constexpr bool operator==(const CaPS_SA& lhs, const CaPS_SA& rhs) {
        return lhs.n_ == rhs.n_ && lhs.p_ == rhs.p_ &&
