...
namespace lz {
  ...
  auto general_cosine(int M, std::vector<double> a) -> std::vector<double> {
    if (M <= 1) return std::vector<double>(M, 1.0);
   ...
   * @return std::vector<double> The generated Hamming window values
   */
  auto hamming(int M) -> std::vector<double> { return general_hamming(M, 0.54); }
   ...
   */
  auto hann(int M) -> std::vector<double> { return general_hamming(M, 0.5); }
  ...
  std::vector<Cpx> fft_recursive(const std::vector<Cpx>& signal) {
    ...
    auto s = signal;
    for (auto i = 0ul; i < n - signal.size(); i++) s.emplace_back(0., 0.);
   ...
   */
  std::map<std::string, std::function<std::vector<double>(int)>> windows
    = {{"hamming", hamming}, {"hamm", hamming}, {"ham", hamming}, {"hann", hann}, {"han", hann}};
  ...
  std::pair<std::vector<double>, lz_size> ShuffleSpectralSignal(const Signal& signal, lz_int block_size = 0) {
    ...
    if (mm <= 0) {
      mm = utils::max_block_size(
        signal.signal.size());                   // the maximum number for the sum in the entropy estimation
  ...
  auto effective_spectral_complexity(const Signal& signal,
    ...
    std::for_each(H_rand.begin(), H_rand.end(), [&](auto&& idx) {
      eff_spectral_cpx += H_rand[idx] - spectral_entropy_h;
    });
