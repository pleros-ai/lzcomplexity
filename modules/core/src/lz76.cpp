...
namespace lz {
  ...
  namespace internal {
    ...
    LZ_Result LempelZiv76::Factorize(const utils::LZ_SuffixArray _SA) {
      ...
      auto logn = std::log(details::ALPHABET_SIZE);
      epsilon = 2 * (1 + std::log(std::log(details::ALPHABET_SIZE * _SA.SA.size()) / logn) / logn)
    ...
    LZ_Result LempelZiv76::Factorize(const char* s, int N) {
      ...
      std::vector<lz_uint> factors{0, 1};
    ...
    LZ_Result LempelZiv76::Factorize(const sequence& seq, utils::LZ_Args& sa_args) {
      ...
      const auto log_base
        = sa_args.log_base == details::NO_ALPHABET ? seq.getAlphabetSize() : sa_args.log_base;
      const auto alphabet
        = sa_args.alphabet == details::NO_ALPHABET ? seq.getAlphabetSize() : sa_args.alphabet;

      auto logn = std::log(log_base);
      epsilon
        = 2 * (1 + std::log(std::log(alphabet * seq.size()) / logn) / logn) / (std::log(seq.size()) / logn);
    ...
    lz_double LempelZiv76::FoundStddev() {
      ...
      for (auto i = 1ul; i < lzf.size(); i++) {
        ...
        if (size > max_factor_size) max_factor_size = size;
      }
      ...
      // lz_double sq_sum =
      //     std::transform_reduce(PAR diff.begin(), diff.end(), 0.0, std::plus{}, [](auto val) { return val *
      //     val;
      //     });
