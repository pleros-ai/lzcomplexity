...
namespace lz {
    namespace utils {
        ...
        struct SA_Args {
            lz_int chunks;          //!> Number of chunks to divide the sequence (use by CaPS)
            lz_int max_context;     //!> Max length to compare prefix for generate the LCP (use by CaPS)

            SA_Args() : chunks(0), max_context(0) {};
            SA_Args(lz_int chunks, lz_int max_context) : chunks(chunks), max_context(max_context) {};
