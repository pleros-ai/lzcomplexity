...
namespace lz {
    ...
    * @param output Output container where shuffle complexity will be stored in `data[i].random_shuffle_complexity`.
    * @return 0 on success, non-zero error code on failure.
    *
    ...
    * normalized information distance: d = [C(S_i S_{i+1}) - min(C(S_i), C(S_{i+1}))] / max(C(S_i), C(S_{i+1})).
    *
