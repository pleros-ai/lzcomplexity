   ...
   namespace utils {
      ...
      struct LZ_Flags {
         ...
         LZ_Flags(sa_type<>::type& sa_impl_, std::string text, LZ_Args _sa_args)
             : sa_impl(sa_impl_), input({text}), alphabet_size(2), sa_args(_sa_args) {
            if (std::holds_alternative<lz::suffixarray::CaPS_SA>(sa_impl_)) {
               algorithm = SA_ALG::caps;
            } else {
               algorithm = SA_ALG::sais;
            }
         };
         ...
         LZ_Flags(sa_type<>::type& sa_impl_, std::vector<sequence> data, LZ_Args _sa_args)
             : sa_impl(sa_impl_), input(data), alphabet_size(2), sa_args(_sa_args) {
            if (std::holds_alternative<lz::suffixarray::CaPS_SA>(sa_impl_)) {
               algorithm = SA_ALG::caps;
            } else {
               algorithm = SA_ALG::sais;
...
#endif
         /** @deprecated */
         [[deprecated]] LZ_Flags(SA_ALG algorithm, std::string text, LZ_Args _sa_args)
             : algorithm(algorithm), input({text}), alphabet_size(2), sa_args(_sa_args){};
         // Copy construct
         ...
         friend constexpr bool operator==(const LZ_Flags& lhs, const LZ_Flags& rhs) {
            return lhs.algorithm == rhs.algorithm && lhs.input == rhs.input && lhs.sa_args == rhs.sa_args;
         };
