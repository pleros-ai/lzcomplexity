...
namespace lz {
   namespace utils {
      ...
      struct LZ_Output {
         ...
         std::vector<lz_bool> calculated_complexity;  ///< Flags indicating if complexity was computed for each sequence.

         std::vector<lz_uint>   half_complexity;          ///< LZ76 complexity of the first half of each sequence.
         std::vector<lz_double> lz_effective_complexity;  ///< Effective complexity (excess entropy via mutual information).
         ...
         std::vector<lz_double> random_shuffle_distance;  ///< Shuffle-based information distance between consecutive sequences.
         std::vector<lz_double> sequence_info_distance;   ///< Information distance computed within each sequence.

         std::vector<LZ_Extra> extra;  ///< Additional measures (Rajski distance, uncertainties, redundancy, etc.).
       ...
       */
      struct LZ_Flags {
         lz_int shuffle_init_line = LZ_Args::UNDEFINED_LINES;  ///< Starting line index for shuffle entropy deficit calculation (-1 = undefined).
         lz_int shuffle_end_line  = LZ_Args::UNDEFINED_LINES;  ///< Ending line index for shuffle entropy deficit calculation (-1 = undefined).
