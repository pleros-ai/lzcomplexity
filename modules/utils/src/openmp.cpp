...
namespace lz {
   ...
   namespace utils {
      ...
      lz_size num_workers() {
         return static_cast<lz_size>(omp_get_max_threads());
      }

      lz_size worker_id() {
         return static_cast<lz_size>(omp_get_thread_num());
      }
...
#pragma omp parallel for schedule(dynamic)
      ...
      ReturnType parallel_reduce_impl(
         ...
         const lz_size chunk_size = (range_size + static_cast<lz_size>(num_threads) - 1) / 
                                    static_cast<lz_size>(num_threads);
