...
namespace lz {
  ...
  namespace utils {
    ...
    std::ostream& WriteBin(std::ostream& os, const sequence& obj) {
      ...
      while (i < obj.size()) {
        c = c << 1;
        if (obj.at(i)) ++c;  // adding 1 if bit is true
        ++bits;
     ...
     * @throws None
     */
    unsigned int WriteNonBin(std::ostream&   os,
                             const sequence& obj,
    ...
    std::ostream& pnm::SavePGM(std::ostream&                             os,
      ...
      if (newfile > 0) {
        ...
        if (maxv == 0) maxvalue = *std::max_element(start, end);
        else maxvalue = maxv;
    ...
    std::istream& pnm::ReadRAW(std::istream& is, sequence& s, bool bin) {
      ...
      try {
        if (bin) magick_number = PNM_RAWBIN;
        else magick_number = PNM_RAWTXT;
