...
namespace lz {
  namespace utils {
    ...
    lz_size num_workers() { return static_cast<lz_size>(internal::CPU_Bandwidth()); }

    lz_size worker_id() {
      auto id = tbb::this_task_arena::current_thread_index();
      return id == tbb::task_arena::not_initialized ? 0 : static_cast<lz_size>(id);
    }
      ...
      arena->Access().execute([&] {
        tbb::this_task_arena::isolate([&] {
          result = tbb::parallel_reduce(
            internal::LZ_BlockedRange<lz_size>(init, end), init_value, fun, reduce_fun);
        });
      });
      return result;
    }
...
template std::vector<lz::sequence> lz::internal::parallel_reduce_impl<std::vector<lz::sequence>>(
  ...
  const std::function<std::vector<lz::sequence>(internal::LZ_BlockedRange<lz_size>,
                                                std::vector<lz::sequence>)>&,
