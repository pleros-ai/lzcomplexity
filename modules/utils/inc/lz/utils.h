...
namespace lz {
  namespace utils {
    ...
    inline lz_double log(auto num, auto base = details::ALPHABET_SIZE) {
...
#ifdef __cpp_lib_ranges
      auto view = s | std::views::drop_while(is_space) | std::views::reverse
    ...
    template<typename T> inline T* allocate(unsigned int size) {
    ...
    constexpr uint32_t hash(std::string_view data) noexcept {
      ...
      for (const char c: data) hash = ((hash << 5) + hash) + (unsigned char)c;
