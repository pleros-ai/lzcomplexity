...
void PyCaps(py::module& m) {
   ...
   caps.def(py::init<const char*, lz::lz_int, lz::lz_int, lz::lz_int>(), "T"_a, "n"_a, "subproblem_count"_a = 0,
            "max_context"_a = 0)
       // .def(py::init<lz::lz_int, lz::lz_int>(), "subproblem_count"_a = 0, "max_context"_a = 0)
       .def(
           "construct", [](suf::CaPS_SA& self) { return self.construct(); },
           "Generate the suffix array from the text use for build the class", py::return_value_policy::copy)
       .def("construct", py::overload_cast<const std::string&>(&suf::CaPS_SA::construct),
            "Generate the suffix array from the text", py::return_value_policy::copy);
...
void PySais(py::module& m) {
   ...
   sais.def(py::init<const char*, lz::lz_int>(), "T"_a, "n"_a)
       .def(
           "construct", [](suf::SAIS& self) { return self.construct(); },
           "Generate the suffix array from the text use for build the class", py::return_value_policy::copy)
       .def("construct", py::overload_cast<const std::string&>(&suf::SAIS::construct),
            "Generate the suffix array from the text", py::return_value_policy::copy);
...
void PySaStructure(py::module& m) {
   ...
   LZ_SuffixArray.def(py::init())
       ...
       .def(
           "__deepcopy__", [](const utl::LZ_SuffixArray& self, py::dict) { return utl::LZ_SuffixArray(self); },
           "memo"_a);
