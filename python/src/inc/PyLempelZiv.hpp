      ...
      if (std::is_same_v<Seq, lz::sequence> == true) {
        ...
        std::visit(overload{[&](auto&& s) { cpx = fun(s, args); },
                            [&](std::vector<int> s) {
                              auto string_view
                                = s | std::views::transform([](int num) { return std::to_string(num); });
                              auto str = std::accumulate(std::ranges::begin(string_view),
                                                         std::ranges::end(string_view),
                                                         std::string{});
...
auto generateFunctionSequenceWithArgs(Fun&& fun) {
  return
    [fun = std::forward<Fun>(fun)](const lz::sequence& seq, utl::LZ_Args& args) { return fun(seq, args); };
}
...
auto generateFunctionSequenceWithArgsForDistance(Fun&& fun) {
  return
    [fun = std::forward<Fun>(fun)](const lz::sequence& seq1, const lz::sequence& seq2, utl::LZ_Args& args) {
      T cpx;
