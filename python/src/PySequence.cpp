...
void PySequence(py::module_& m) {
   ...
   sequence.def(py::init())
      .def(py::init<lz::lz_int>(), "alphabet_size"_a, "Initialize sequence with given alphabet size")
      .def(py::new_(generateSequenceConstructor()), "str"_a, "Initialize sequence from string")
      .def(py::new_(generateSequenceConstructorWithAlphabet()),
           "str"_a,
           "alphabet_size"_a,
           "Initialize sequence from string with given alphabet size")
      .def(py::self + py::self, "Concatenate two sequences")
      .def(py::self += py::self, "Append another sequence")
      .def(py::self += std::string(), "Append a string")
      .def(py::self += std::vector<lz::lz_char>(), "Append a vector of characters")
      .def(py::self == py::self, "Compare two sequences for equality")
      .def(py::self == std::string(), "Compare sequence with string for equality")
      .def(py::self != py::self, "Compare two sequences for inequality")
      .def(py::self != std::string(), "Compare sequence with string for inequality")
      .def(py::self > py::self, "Compare if sequence is greater than another")
      .def(py::self > std::string(), "Compare if sequence is greater than string")
      .def(py::self >= py::self, "Compare if sequence is greater or equal than another")
      .def(py::self >= std::string(), "Compare if sequence is greater or equal than string")
      .def(py::self < py::self, "Compare if sequence is less than another")
      .def(py::self < std::string(), "Compare if sequence is less than string")
      .def(py::self <= py::self, "Compare if sequence is less or equal than another")
      .def(py::self <= std::string(), "Compare if sequence is less or equal than string")
      .def("__str__", &::toString, "Convert sequence to string")
      .def("__getitem__", py::overload_cast<lz::lz_size>(&lz::sequence::operator[]), "Get character at index")
